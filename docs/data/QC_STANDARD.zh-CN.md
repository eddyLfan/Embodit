...
下表是完整数据治理的目标分层，不等于当前版本已经覆盖全部层级。Embodit 目前主要实现 L0、L2 和部分 L3 检查；完整时序同步、设备物理限位、任务成功、重复/漂移和 train/eval 泄漏仍需设备 Profile、外部证据或后续实现。页面中的质量分和自动决定不得解释为对未覆盖层级的合格证明。
