...
import deploy.cli as deploy_cli
...
from deploy.cli import promote_live_interactively
...
def raw_recipe() -> dict:
    ...
    # Keep credential and path behavior tests independent from the safe public
    # example values. The committed Recipe intentionally contains no secrets.
    raw["hosts"]["model"]["auth"] = {
        "type": "password",
        "password": "CHANGE_ME_MODEL_PASSWORD",
    }
    raw["hosts"]["robot"]["auth"] = {
        "type": "password",
        "password": "CHANGE_ME_ROBOT_PASSWORD",
    }
    raw["model"]["workdir"] = "/root/vla"
    raw["model"]["checkpoint"] = "/root/checkpoints/my-vla"
    raw["model"]["python_executable"] = "/root/miniconda3/envs/vla/bin/python"
    return raw
...
def test_robot_config_can_be_freely_paired_with_a_different_model() -> None:
    ...
    second_model.host.address = "192.0.2.199"
    ...
    assert recipe.hosts["model"].address == "192.0.2.199"
...
def test_recipe_default_live_cannot_bypass_dry_run(tmp_path: Path) -> None:
    raw = raw_recipe()
    raw["runtime"]["default_mode"] = "live"
    orchestration = HarnessOrchestration(
        parse_recipe(raw),
        tmp_path,
        runner_factory=lambda _name, _host: FakeRunner(),
...
def test_failed_live_client_switch_stops_new_client_and_restores_dry_run(
    tmp_path: Path,
) -> None:
    ...
    previous_config = json.loads(json.dumps(orchestration.recipe.robot.client.config))
    started_modes = []
    health_checks = 0

    def start_client():
        started_modes.append(orchestration.mode)
        orchestration.components["client"]["active"] = True
    ...
    def wait_client_health():
        ...
        health_checks += 1
        if health_checks == 1:
            raise RuntimeError("new live client unhealthy")
        orchestration.state = OrchestrationState.STOPPED
        orchestration.mode = "mutated-by-health-check"
        orchestration.recipe.robot.client.config = {"mutated": True}

    orchestration._start_client = start_client
    orchestration._wait_client_health = wait_client_health

    challenge = orchestration.arm_challenge()
    with pytest.raises(RuntimeError, match="new live client unhealthy"):
        orchestration.promote_live(challenge["phrase"])

    assert manager.stopped == ["client", "client"]
    assert started_modes == ["live", "dry_run"]
    assert orchestration.state == OrchestrationState.DRY_RUN
    assert orchestration.mode == "dry_run"
    assert orchestration.components["client"]["active"] is True
    assert orchestration.recipe.robot.client.config == previous_config


def test_failed_live_client_switch_faults_when_previous_client_cannot_recover(
    tmp_path: Path,
) -> None:
    class RecordingManager:
        def __init__(self):
    ...
    orchestration._start_client = start_client
    orchestration._wait_client_health = lambda: (_ for _ in ()).throw(
        RuntimeError("client health failed")
    )

    challenge = orchestration.arm_challenge()
    with pytest.raises(RuntimeError, match="恢复原 Client 失败"):
        orchestration.promote_live(challenge["phrase"])

    assert manager.stopped == ["client", "client", "client"]
    assert started_modes == ["live", "dry_run"]
    assert orchestration.state == OrchestrationState.FAULT
    assert orchestration.mode == "dry_run"
    assert orchestration.components["client"]["active"] is False
    assert orchestration.components["model"]["active"] is True
    assert orchestration.recipe.robot.client.config == previous_config
    assert "恢复原 Client 失败" in orchestration.last_error


def test_live_requires_fresh_challenge_and_exact_confirmation(tmp_path: Path) -> None:
    orchestration = DeploymentOrchestration(
        parse_recipe(python_adapter_recipe()),
        tmp_path,
        runner_factory=lambda _name, _host: FakeRunner(),
    ...
    with pytest.raises(ValueError, match="无效或已过期"):
        orchestration.promote_live("LIVE robot-a--my-vla guessed")
    ...
    challenge = orchestration.arm_challenge()
    snapshot = orchestration.promote_live(challenge["phrase"])
...
def test_cli_live_confirmation_rejects_noninteractive_input() -> None:
    class LiveItem:
        def __init__(self):
            self.challenge_requests = 0

        def arm_challenge(self):
            self.challenge_requests += 1
            return {"phrase": "LIVE demo ABC123", "expiresInSeconds": 60}

    item = LiveItem()
    with pytest.raises(ValueError, match="交互式终端"):
        promote_live_interactively(item, input_stream=io.StringIO("LIVE demo ABC123\n"))
    assert item.challenge_requests == 0


def test_cli_live_mode_fails_before_starting_services_without_tty(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    recipe_path = tmp_path / "recipe.json"
    recipe_path.write_text(json.dumps(raw_recipe()), encoding="utf-8")
    constructed = False

    def construct(*_args, **_kwargs):
        nonlocal constructed
        constructed = True
        raise AssertionError("orchestration must not be constructed")

    monkeypatch.setattr(deploy_cli, "DeploymentOrchestration", construct)
    monkeypatch.setattr(deploy_cli.sys, "argv", [
        "embodit-recipe", "run", str(recipe_path), "--mode", "live",
    ])
    monkeypatch.setattr(deploy_cli.sys, "stdin", io.StringIO())
...
def test_cli_live_confirmation_forwards_exact_server_phrase() -> None:
    ...
    class LiveItem:
        ...
        def promote_live(self, confirmation):
            self.confirmations.append(confirmation)
            return {"state": "running", "mode": "live"}

    item = LiveItem()
    output = io.StringIO()
    snapshot = promote_live_interactively(
        item,
        input_stream=InteractiveInput("LIVE demo ABC123\n"),
        output_stream=output,
    )

    assert snapshot == {"state": "running", "mode": "live"}
    assert item.confirmations == ["LIVE demo ABC123\n"]
    assert "LIVE demo ABC123" in output.getvalue()
...
def test_fault_with_active_components_must_be_cleaned_before_restart(tmp_path: Path) -> None:
    orchestration = DeploymentOrchestration(
        parse_recipe(raw_recipe()),
        tmp_path,
        runner_factory=lambda _name, _host: FakeRunner(),
    )
    orchestration.state = OrchestrationState.FAULT
    orchestration.components["model"]["active"] = True

    with pytest.raises(ValueError, match="仍有活动组件"):
        orchestration.start()
    with pytest.raises(ValueError, match="仍有活动组件"):
        orchestration.prepare_model()


def test_registry_treats_fault_with_active_components_as_active(tmp_path: Path) -> None:
    registry = OrchestrationRegistry(tmp_path)
    existing = registry.create(raw_recipe())
    existing.state = OrchestrationState.FAULT
    existing.components["model"]["active"] = True

    with pytest.raises(ValueError, match="已有活动编排"):
...
def test_registry_stop_all_cleans_fault_with_active_components(tmp_path: Path) -> None:
    ...
    registry = OrchestrationRegistry(tmp_path)
    item = registry.create(raw_recipe())
    manager = RecordingManager()
    item._managers[item.robot_host_name] = manager
    item.state = OrchestrationState.FAULT
    item.components["client"]["active"] = True
...
def test_orchestration_offline_inference_uses_resident_model_endpoint(tmp_path: Path) -> None:
    class OfflineInferenceRunner:
        ...
        def run(self, args, **kwargs):
            ...
            return RemoteResult(0, json.dumps({"action": {"values": [[1.0, 2.0]]}}))

    recipe = raw_recipe()
    recipe["model"]["startup_timeout_s"] = 600
    orchestration = DeploymentOrchestration(
        parse_recipe(recipe),
        tmp_path,
        runner_factory=lambda _name, _host: FakeRunner(),
    )
    runner = OfflineInferenceRunner()
    orchestration._runners[orchestration.model_host_name] = runner
    orchestration.state = OrchestrationState.MODEL_READY
    orchestration.components["model"]["active"] = True

    result = orchestration.infer_observations({"observation.state": [0.1, 0.2]})

    assert result["action"]["values"] == [[1.0, 2.0]]
    args, kwargs = runner.call
    assert f"http://127.0.0.1:{orchestration.recipe.tunnel.remote_port}/infer" in args
    assert float(args[-2]) == 30.0
    assert kwargs["timeout"] == 35.0
    request = json.loads(kwargs["input_data"])
    assert request["mode"] == "offline_evaluation"
    assert request["observations"]["observation.state"] == [0.1, 0.2]


def test_orchestration_offline_inference_is_single_flight_and_blocks_robot_start(
    tmp_path: Path,
) -> None:
    started = threading.Event()
    release = threading.Event()

    class BlockingInferenceRunner:
        def __init__(self):
            self.calls = 0
    ...
    runner = BlockingInferenceRunner()
    orchestration._runners[orchestration.model_host_name] = runner
    orchestration.state = OrchestrationState.MODEL_READY
    orchestration.components["model"]["active"] = True
    result = {}
    ...
    worker = threading.Thread(
        target=lambda: result.update(
            orchestration.infer_observations({"observation.state": [0.1, 0.2]})
        )
    )
    worker.start()
    assert started.wait(timeout=2)

    with pytest.raises(ValueError, match="离线评测正在进行"):
        orchestration.infer_observations({"observation.state": [0.3, 0.4]})
    with pytest.raises(ValueError, match="不能启动本体链路"):
        orchestration.start(task_prompt="pick the cube")
    with pytest.raises(ValueError, match="不能开始真机评测"):
        orchestration.start_evaluation(task_prompt="pick the cube")

    assert orchestration.state == OrchestrationState.MODEL_READY
    assert orchestration.recipe.robot.client.config.get("task_prompt") is None
...
def test_orchestration_offline_inference_releases_single_flight_after_failure(
    tmp_path: Path,
) -> None:
    class SequenceInferenceRunner:
        def __init__(self):
            self.calls = 0
        ...
        def run(self, _args, **_kwargs):
            self.calls += 1
            if self.calls == 1:
                return RemoteResult(1, "", "temporary failure")
            return RemoteResult(0, json.dumps({"action": {"values": [[1.0, 2.0]]}}))
    ...
    runner = SequenceInferenceRunner()
    orchestration._runners[orchestration.model_host_name] = runner
    orchestration.state = OrchestrationState.MODEL_READY
    orchestration.components["model"]["active"] = True
...
def test_model_only_precheck_does_not_connect_to_robot(tmp_path: Path) -> None:
    class HostProbeRunner:
        ...
        def run(self, args, **kwargs):
            self.calls.append((args, kwargs))
            if "platform.node" in args[2]:
                return RemoteResult(0, json.dumps({
                    "hostname": self.name,
                    "python": "3.12",
            ...
            return RemoteResult(0, f"/home/{self.name}\n")

    runners = {}

    def factory(name, _host):
        runners[name] = HostProbeRunner(name)
        return runners[name]

    orchestration = DeploymentOrchestration(parse_recipe(raw_recipe()), tmp_path, runner_factory=factory)
    orchestration._precheck_model_only()

    assert runners[orchestration.robot_host_name].calls == []
    assert len(runners[orchestration.model_host_name].calls) == 2
    assert orchestration._model_home == f"/home/{orchestration.model_host_name}"
    assert orchestration._robot_home is None
...
def test_web_workspace_keeps_deployment_config_read_only() -> None:
    ...
    assert 'id="deploymentOfflineDataset"' in html
    assert 'id="deploymentOfflineEpisode"' in html
    assert 'id="deploymentOfflineFrame"' in html
    assert 'id="runDeploymentOfflineEvaluation"' in html
    assert 'id="deploymentOfflineResult"' in html
    ...
    assert "/start-evaluation" not in javascript
    assert "/offline-evaluation" in javascript
