...
from qc import fingerprint as fingerprint_module
...
def test_dataset_fingerprint_tracks_lerobot_parquets(tmp_path: Path) -> None:
    cases = (
        ("lerobot_v21", "data/chunk-000/episode_000000.parquet"),
        ("lerobot_v3", "data/chunk-000/file-000.parquet"),
        ("lerobot_v3", "meta/episodes/chunk-000/file-000.parquet"),
    )
    for index, (format_id, relative) in enumerate(cases):
        dataset = tmp_path / f"dataset-{index}"
        parquet = dataset / relative
        parquet.parent.mkdir(parents=True)
        parquet.write_bytes(b"before")
        view = SimpleNamespace(format_id=format_id, fps=30.0, features={}, episodes=[])

        before = fingerprint_module.dataset_fingerprint(dataset, view)
        parquet.write_bytes(b"after-with-a-different-size")

        assert fingerprint_module.dataset_fingerprint(dataset, view) != before


def test_dataset_fingerprint_stats_shared_episode_files_once(monkeypatch, tmp_path: Path) -> None:
    dataset = tmp_path / "dataset"
    shared_video = dataset / "videos" / "shared.mp4"
    ...
    view = SimpleNamespace(format_id="fake", fps=30.0, features={}, episodes=episodes)
    calls: dict[str, int] = {}
    original = fingerprint_module._stat_payload

    def counted_stat(path: Path):
        calls[str(path)] = calls.get(str(path), 0) + 1
        return original(path)

    monkeypatch.setattr(fingerprint_module, "_stat_payload", counted_stat)

    fingerprint_module.dataset_fingerprint(dataset, view)

    assert calls[str(shared_video)] == 1
...
def test_intervals_discard_ranges_outside_episode_bounds() -> None:
    assert mask_to_intervals([False, False, True], 1.0, duration=1.0) == []
    assert union_duration([(-3.0, -1.0), (4.0, 8.0)], duration=3.0) == 0.0
    assert union_duration([(-1.0, 1.0), (2.0, 5.0)], duration=3.0) == 2.0
...
def test_intervals_reject_nonpositive_or_nonfinite_fps(fps: float) -> None:
    with pytest.raises(ValueError, match="fps"):
        mask_to_intervals([True, True], fps)


@pytest.mark.parametrize("endpoint", [float("nan"), float("inf"), float("-inf")])
def test_union_duration_rejects_nonfinite_endpoints(endpoint: float) -> None:
    with pytest.raises(ValueError, match="有限数值"):
        union_duration([(0.0, endpoint)], duration=10.0)


def test_interval_thresholds_do_not_round_across_configured_boundaries() -> None:
    assert mask_to_intervals([True, False], 2.0, minimum_seconds=0.51) == []
    assert mask_to_intervals([True, False], 2.0, minimum_seconds=0.5) == [(0.0, 0.5)]
    assert mask_to_intervals([True, False, True], 2.0, merge_gap_seconds=0.26) == [
        (0.0, 0.5),
        (1.0, 1.5),
    ]
    assert mask_to_intervals([True, False, True], 2.0, merge_gap_seconds=0.5) == [
        (0.0, 1.5)
    ]
...
def test_sqlite_report_query_review_and_selection(tmp_path: Path) -> None:
    ...
    assert report_summary["totals"]["pass"] == 1
    assert report_summary["totals"]["quarantine"] == 1
    ...
    review_episode(report, 0, "pass", "manual override", "tester")
    overridden_summary = summary(report)
    assert overridden_summary["totals"]["pass"] == 2
    assert overridden_summary["totals"]["quarantine"] == 0
    ...
    chunks = list(report_csv_chunks(report, "episodes", batch_size=1))
    assert len(chunks) == 3
    assert chunks[0].startswith("episode_index,")
    assert report_csv(report, "episodes") == "".join(chunks)

    # Starlette may resume a synchronous StreamingResponse iterator on a
    # different worker thread for every next(). Keep those threads alive so
    # this deterministically exercises three distinct thread identities.
    iterator = report_csv_chunks(report, "episodes", batch_size=1)
    gates = [threading.Event() for _ in range(4)]
    release = threading.Event()
    threaded_chunks: list[str | BaseException | None] = [None, None, None]

    def consume_chunk(index: int) -> None:
        gates[index].wait(timeout=3)
        try:
            threaded_chunks[index] = next(iterator)
        except BaseException as error:  # pragma: no cover - assertion reports it
            threaded_chunks[index] = error
        finally:
            gates[index + 1].set()
            release.wait(timeout=3)

    threads = [threading.Thread(target=consume_chunk, args=(index,)) for index in range(3)]
    for thread in threads:
        thread.start()
    gates[0].set()
    assert gates[3].wait(timeout=3)
    release.set()
    for thread in threads:
        thread.join(timeout=3)

    assert threaded_chunks == chunks
