...
def test_migrate_legacy_layout_and_rewrite_job_paths(tmp_path: Path) -> None:
    ...
    assert result["moved"]
    assert (layout.convert_jobs / "abc.json").is_file()
    ...
    rewritten = json.loads((layout.convert_jobs / "abc.json").read_text(encoding="utf-8"))
    assert rewritten["logPath"] == str(layout.convert_jobs / "abc.log")


def test_migrate_removes_old_compatibility_links(tmp_path: Path) -> None:
    project = tmp_path / "project"
    layout = CacheLayout.under(project / ".embodit_cache")
    _write(layout.convert_jobs / "existing.json", "{}")
    _write(layout.qc_jobs / "existing.json", "{}")
    old_jobs = project / ".convert_jobs"
    old_qc_jobs = layout.root / "qc_jobs"
    old_jobs.symlink_to(layout.convert_jobs, target_is_directory=True)
    ...
    assert (layout.convert_jobs / "existing.json").is_file()
